...
interface TodoProps {
  todo: TodoData
  onCheckTodo: (todo: TodoData, isChecked: string | boolean) => void
...
export function Todo({ todo, onCheckTodo, onDeleteTodo }: TodoProps) {
  ...
  function handleCheckTodo(checked: string | boolean) {
    if (checked) {
      setIsChecked(true)
    } else {
      setIsChecked(false)
    ...
    <div className={styles.todo}>
      <div className={styles.todoContent}>
        <Checkbox.Root
          id={todo.id}
          checked={isChecked} 
          onCheckedChange={checked => handleCheckTodo(checked)}  
          className={isChecked ? styles.checkboxChecked : styles.checkboxUnchecked}
        >
          <Checkbox.Indicator className={styles.checkmarkContainer}>
        ...
        <label htmlFor={todo.id} className={styles.checkboxLabel}>{todo.content}</label>
      </div>
